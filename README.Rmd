...
The goal of BayesMassBal is to allow users to do Bayesian data reconciliation for a linearly constrained chemical or particulate process at steady state.
...
```{r multiprocess, echo = FALSE, fig.cap = "Example two node circuit"}
...
Code for the data simulation creates a normally distributed feed rate, process variability at each node drawn from independent beta distributions, and *assay noise* on top of each observed mass.
...
More typically, the `massbal.data()` function can be used to import data collected in the field and collated into a .csv.  `massbal.data()` also organizes user collected data for use with the `bayes.massbal()` function.
