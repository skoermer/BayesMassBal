...
#' @details The output of this function is meant to be used as the input parameter \code{X} in \code{\link{BMB}}.  The matrix \code{C}, or imported matrix from \code{file}, indexes sampling locations via columns, and number of constraints via rows. Only values of -1, 0, and 1 are valid, and indicate mass leaving a node, a location that is not relevant to a node, and mass entering a node respectively.  Constraints only need to be indicated around each node.  Additional constraints are redundant.  Each sample component is subject to the same constraints, and therefore the constraints given to \code{constrainProcess} do not need to be repeated for each component.  A quick example, \code{C} for a single process, where the constraint is \eqn{y_1 = y_2 + y_3}, and there are two sample components, would be \code{matrix(c(1,-1,-1), nrow=  1,ncol = 3}).
